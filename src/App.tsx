...
const lotDescriptor = (lot: Pick<InventoryLot, 'cardFormat' | 'gradingCompany' | 'grade' | 'condition'>) => lot.cardFormat === 'graded' ? `${lot.gradingCompany ?? 'Graded'} ${lot.grade ?? ''}`.trim() : lot.condition
  ...
  return <>
    ...
    <section className="content-grid">
      ...
      <div className="panel inventory-glance">
        ...
        <div className="mini-list">{state.lots.slice(0, 3).map((lot) => <div key={lot.lotId}><CardThumb lot={lot} /><span><strong>{lot.name}</strong><small>{lot.setCode} · #{lot.number} · {lot.variant ?? 'Unspecified'}</small></span><b>×{lot.quantity}</b></div>)}</div>
      </div>
    </section>
  </>
}
...
function Inventory({ lots, total, filter, setFilter, onBuy, onSell, onTrade, onSellLot, onEditLot, onRemoveLot }: { lots: InventoryLot[]; total: number; filter: string; setFilter: (s: string) => void; onBuy: () => void; onSell: () => void; onTrade: () => void; onSellLot: (lot: InventoryLot) => void; onEditLot: (lot: InventoryLot) => void; onRemoveLot: (lot: InventoryLot) => void }) {
  return <>
    <section className="page-title"><div><p className="eyebrow">YOUR STOCK</p><h1>Inventory</h1><p>{total} lots, organized by acquisition cost.</p></div><div className="desktop-actions"><button className="action buy" onClick={onBuy}><Plus /> Add cards</button><button className="action trade" onClick={onTrade}><Handshake /> Trade</button><button className="action sell" onClick={onSell}><ArrowUpRight /> Record sale</button></div></section>
    ...
    <section className="inventory-table panel">
      <div className="table-head"><span>Card</span><span>Condition</span><span>Qty.</span><span>Unit cost</span><span>Market</span><span>Potential</span><span>Actions</span></div>
...
function TradeModal({ lots, onClose, onSave }: { lots: InventoryLot[]; onClose: () => void; onSave: (trade: TradeEvent, incomingLots: InventoryLot[]) => void }) {
  ...
  const canSave = outgoing.length > 0 && incoming.length > 0 && incoming.every((item) => item.format === 'raw' || (item.gradingCompany?.trim() && item.grade?.trim())) && Number(cashPaid) >= 0 && Number(cashReceived) >= 0
  ...
  function saveTrade() {
    ...
    const incomingLots: InventoryLot[] = incoming.map((item) => {
      ...
      const graded = item.format === 'graded'
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal trade-modal" role="dialog" aria-modal="true">
    <div className="modal-head"><div><p className="eyebrow">CARD FOR CARD</p><h2>Record a trade</h2></div><button onClick={onClose}><X /></button></div>
    <div className="trade-body">
      <div className="trade-columns">
        <section className="trade-side outgoing"><div className="trade-side-title"><span><ArrowUpRight /></span><div><h3>You give</h3><p>Cards leaving inventory</p></div></div>
        ...
        <section className="trade-side incoming"><div className="trade-side-title"><span><ArrowDownLeft /></span><div><h3>You receive</h3><p>Cards entering inventory</p></div></div>
          ...
          <div className="trade-search"><Search /><input value={inQuery} onChange={(event) => setInQuery(event.target.value)} placeholder="Search the card catalog…" />{searching && <span className="spinner" />}</div>
...
function TransactionModal({ flow, lots, initialLot, onClose, onBuy, onSell }: { flow: Flow; lots: InventoryLot[]; initialLot?: InventoryLot | null; onClose: () => void; onBuy: (card: CardResult, quantity: number, price: number, condition: Condition, variant?: CardVariant, details?: PurchaseDetails) => void; onSell: (lot: InventoryLot, quantity: number, price: number) => void }) {
  ...
  function submit() {
    if (!valid) return
    if (flow === 'buy' && selectedCard) onBuy(selectedCard, quantity, Number(price), condition, selectedVariant, { format: cardFormat, gradingCompany: cardFormat === 'graded' ? gradingCompany.trim() : undefined, grade: cardFormat === 'graded' ? grade.trim() : undefined, certificationNumber: cardFormat === 'graded' ? certificationNumber.trim() || undefined : undefined, notes: notes.trim() || undefined, gradedMarketValue: cardFormat === 'graded' ? Number(gradedMarketValue) : undefined })
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><div className="modal" role="dialog" aria-modal="true">
    ...
    {!chosen ? <div className="finder">
      ...
      {flow === 'buy' && <div className="format-toggle"><button className={cardFormat === 'raw' ? 'active' : ''} onClick={() => setCardFormat('raw')}>Raw card</button><button className={cardFormat === 'graded' ? 'active' : ''} onClick={() => setCardFormat('graded')}>Graded slab</button></div>}
