...
const columns = ['show_name', 'show_date', 'record_type', 'transaction_type', 'product_name', 'card_number', 'set_name', 'product_type', 'variant', 'card_format', 'grading_company', 'grade', 'certification_number', 'condition', 'quantity', 'unit_cost', 'unit_price', 'market_price', 'market_price_source', 'total', 'profit', 'notes', 'timestamp'] as const
...
export function exportLedgerCsv(state: LedgerState) {
  const inventoryRows = state.lots.map((lot) => {
    ...
    return [
      ...
      lot.productType ?? 'card', lot.variant ?? 'Unspecified', lot.cardFormat ?? 'raw', lot.gradingCompany, lot.grade, lot.certificationNumber,
  ...
  const transactionRows = state.transactions.map((transaction) => [
    ...
    transaction.cardNumber, transaction.setName, transaction.productType ?? 'card', transaction.variant ?? 'Unspecified', transaction.cardFormat ?? 'raw',
  ...
  const tradeRows = (state.trades ?? []).flatMap((trade) => [
    ...trade.outgoing.map((line) => [
      ...
      line.productType ?? 'card', line.variant, line.cardFormat ?? 'raw', line.gradingCompany, line.grade, line.certificationNumber, line.condition, line.quantity, line.unitCost.toFixed(2), line.unitTradeValue.toFixed(2), '', '',
